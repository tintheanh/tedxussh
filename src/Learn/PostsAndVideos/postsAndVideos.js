...
class PostListAndPage extends React.Component {
  ...
  groupPost(posts) {
    if (posts !== undefined) {
      ...
      for (let index = 0; index < posts.length; index++) {
        ...
        if (index === posts.length - 1 && posts.length % 6 !== 0) result.push(group)
      }
      return result
    }
    return null
  ...
  render() {
    const { isVN, videoSection, postSection } = this.props
    return (
      <div className="site-section">
        <div className="container">
          {this.state.videoSection !== null ? <VideoSection isVN={isVN} videoSection={videoSection} /> : null}
          <PostSection isVN={isVN} postSection={{header: postSection.header, postList: this.groupPost(postSection.postList)}} />
        </div>
      </div>
    )
  }
